...
#this chunk is to check how data is being arranged in the data sets 
  #filter out rows with CODE = NA since agg entities does not have a CODE associated with them  
...
5. *'malaria_inc'* only gives the data in intervals of 5 years (and only starting from year 2000), compared to the other 2 data sets which gives deaths annually (starting from year 1990). As such, we will only be computing the overall infection mortality rate (see Section XX below) in 5-yearly intervals.
...
[Malaria Deaths & Incidences Data Story](http://example.com)


write.csv(malaria_deaths_combined_regional, "C:/Users/lowzh/Desktop/malaria_deaths_combined_regional.csv", row.names=FALSE)
write.csv(malaria_inc_combined_country_lvl, "C:/Users/lowzh/Desktop/malaria_inc_combined_country_lvl.csv", row.names=FALSE)
